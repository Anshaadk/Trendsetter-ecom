...
def subtotal(current_user):
    ...
    address=AddressDetails.objects.filter(user=current_user)
...
def createorder(newAddress_id,current_user,mode,discount_Display,price,couponCode,total,couponDiscount):
        ...
        if mode == "cod":
            ...
            newPayment.save()
            grand_total = calculateCartTotal(current_user)
            newOrder =Order()
...
def calculateCartTotal(current_user):
